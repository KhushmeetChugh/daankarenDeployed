...
const DonationPaymentPage = () => {
  ...
  const handleDonateNow = async () => {
    ...
    try {
      ...
      const response = await fetch('https://daankarendeployed-1.onrender.com/braintree/payment', {
        method: 'POST',
        headers: {
  ...
  const getToken = async () => {
    try {
      const response = await fetch('https://daankarendeployed-1.onrender.com/braintree/token');
      const data = await response.json();
